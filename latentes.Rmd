...
Para esta sesión trabajaremos con la data del [Indice de Desarrollo Humano](https://es.wikipedia.org/wiki/Anexo:Pa%C3%ADses_por_%C3%ADndice_de_desarrollo_humano)
...
demoL=c("https://en.wikipedia.org/wiki/Democracy_Index", 
        '//*[@id="mw-content-text"]/div[1]/table[10]')
...
## En demo:
# a numerica
demo[,-c(1,2)]=lapply(demo[,-c(1,2)],as.numeric)

# a ordinal
regimeLevels=c("Authoritarian","Hybrid regime","Flawed democracy","Full democracy")
demo$Regime_Type=factor(demo$Regime_Type,
                   levels=regimeLevels, 
                   ordered = T)
# sin perdidos:
demo=na.omit(demo)
...
```{r, echo=TRUE, eval=TRUE,warning=FALSE, message=FALSE}
dontselect=c("Country","Regime_Type","Overall_Score")
...
El problema sería que en la nueva variable aparecen casos 0 y casos 10. Es decir la data debe ser modificada:

```{r}
# creo 5 valores con 10
top=rep(10,5)
# creo 5 valores con 0
bottom=rep(0,5)
# hago data frame con eso:
...
```{r}
model <- ' democracia  =~ Electoral_Process_And_Pluralism + Functioning_Of_Government + Political_Participation + Political_Culture + Civil_Liberties'
```
...
1. Añada otro concepto latente que use varias variables. Siga todos los pasos anteriores.

2. Plantee una regresión Gaussiana, con una "Y" que se una variable latente (varias variables), y dos "X"s que también sean latentes (varias variables por cada "X"). Use las latentes teorizadas (sus 'scores') para la regresión. 