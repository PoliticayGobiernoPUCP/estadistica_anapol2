...
Appending es unir diversas tablas en una sola,pero todas tienen las mismas columnas.
...
Para tener un unico data frame, necesitamos anexarlas:
...
Un caso especial es cuando tienes un archivo en excel
...
```{r}
aggregate(cbind(todaSemana, NoTodaSemana) ~ Departamento, 
          data = agua, sum)
```


```{r}
aggregate(cbind(todaSemana, NoTodaSemana) ~ Departamento + Provincia, 
          data = agua, sum)
```
