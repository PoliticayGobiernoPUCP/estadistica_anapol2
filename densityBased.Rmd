...
```{r, eval=TRUE}
base= ggplot(inputData,aes(x=dim1, y=dim2,label=row.names(inputData)))
base + geom_text(size=2, aes(color=pamCMD))

```

```{r}
base + geom_text(size=2, aes(color=agnesCMD))
```

```{r}
base + geom_text(size=2, aes(color=dianaCMD))
```
```{r}
library(ggrepel)
limites=c(-0.8,0.8)
base + geom_point(aes(color=dbCMD)) + geom_text_repel(size=1) + ylim(limites) + xlim(limites)
```
