...
allData=allData[!allData$Country%in%c("China","United States"),]
...
```{r}
library(BBmisc)
boxplot(BBmisc::normalize(allData[,-1],method='range',range=c(0,1)))
```

```{r}
boxplot(BBmisc::normalize(allData[,-1],method='standardize'))
```

```{r}
### transforming
allData[,-1]=BBmisc::normalize(allData[,-1],method='range',range=c(0,1))
